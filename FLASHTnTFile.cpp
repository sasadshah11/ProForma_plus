...
/// Generate ProForma string with modifications
                                             ...
                                             std::vector<String> mod_ids)
{
  ...
  ProForma proforma(AASequence::fromString(truncated_seq));

  // Validation: Ensure mod_starts and mod_ends are within valid range
  for (size_t i = 0; i < mod_masses.size(); ++i)
  {
    if (mod_starts[i] < seq_start || mod_ends[i] > seq_end)
    {
  ...
  // Add modifications to ProForma
  for (size_t i = 0; i < mod_masses.size(); ++i)
  {
    if (!mod_ids[i].empty())
    {
      proforma.addModification(mod_starts[i] - seq_start, mod_ends[i] - seq_start, mod_ids[i], mod_masses[i]);
    }
  }

  return proforma.toProFormaString();
}
